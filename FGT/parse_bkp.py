...
def parse(args_setting, is_train=True):
    opt_path, name = args_setting['opt'], args_setting['name']
    datadir, outputdir = args_setting['datadir'], args_setting['outputdir']
    ...
    opt['path']['OUTPUT_ROOT'] = name
    # training settings
    if is_train:
        output_root = osp.join(outputdir, opt['path']['OUTPUT_ROOT'], 'experiments', opt['name'])
